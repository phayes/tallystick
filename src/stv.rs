...
impl<T: Eq + Clone + Hash + std::fmt::Debug> Tally<T>  {
    ...
    pub fn result(&mut self) -> IndexMap<T, u32> {
        ...
        // Each loop is a single round, we increase the rank when we find winners
        let mut rank: u32 = 0;
        loop {
            ...
            // Step 2. Count votes 
            let mut votecounts: IndexMap<&T, f64> = IndexMap::new();
            ...
            // Sort desc
            votecounts.sort_by(|_, a, _, b| b.partial_cmp(a).unwrap());

            // Step 4. Check if any candidates are over the threshold, and add them to results
            ...
            for (candidate_ref, count) in votecounts.iter() {
                if *count >= threshold {
                    ...
                    // If we have enough winners, end the tally
                    if (results.len() + new_winners.len()) as u32 >= self.num_winners {
                        for winner in new_winners.drain(0..) {
                            results.insert(winner, rank);
                            rank += 1;
                        }
                        // Check for any additional "equal" winners. 
                        // This is considered a tie and will overflow the ideal number of winners.
                        for (tie_ref, tie_count) in votecounts.iter() {
                            if tie_ref != candidate_ref && tie_count == count {
                                let candidate = *tie_ref;
                                results.insert(candidate.clone(), rank -1); // Needs to be same rank as last candidate to indicate tie.
                            }
                        }
                        return results;
                    }
            ...
            if new_winners.len() > 0 {
                ...
                // We've added winners, continue to the next round.
                continue;
            }
            else {
                // Step 5. Remove loosers and redistribute
...
mod tests {
    ...
    fn stv_wikipedia_test() {
        ...
        assert_eq!(hare_result, indexmap!{"Andrea" => 0, "Carter" => 1, "Delilah" => 2, "Scott" => 2, "Jennifer" => 2});
        assert_eq!(droop_result, indexmap!{"Andrea" => 0, "Carter" => 1, "Brad" => 2, "Scott" => 3, "Delilah" => 3});
        ...
        let hagen_result = hagen_tally.result();
        let droop_result = droop_tally.result();

        // TODO: Return hashmap??
        assert_eq!(hagen_result, indexmap!{"Scott" => 0, "Brad" => 1, "Andrea" => 2, "Jennifer" => 3, "Carter" => 4, "Matt" => 5, "Delilah" => 6});
